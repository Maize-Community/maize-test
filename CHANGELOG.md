...
- This is a hard fork/breaking change from RC6/7. TXMZ Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chia.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `chia farm summary` will now display TXMZ or XMZ as appropriate.
...
- 'chia wallet send' now takes in TXMZ or XMZ as units instead of mojos.
...
- The GUI is now in a separate submodule repository from chia-blockchain, [maize-blockchain-gui](https://github.com/Chia-Network/maize-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `maize-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `chia wallet`, was incorrectly displaying TXMZ balances as if they were Coloured Coins.
...
- The user interface now refers to chia coins with their correct currency code of XMZ.
